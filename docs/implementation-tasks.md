...
**实现状态：静态资源已发布到公开 HTTPS 地址；部署/付款钱包已确认有 120 MON，主办方与平台地址已确认但当前均为 0 MON。** 见 `public/pass.svg`、`public/metadata.json`。
...
- [ ] 付款人准备 3 MON 票款和额外 Gas；部署者、主办方、平台分别有足够操作 Gas。已核实：部署/付款账户 120 MON，主办方账户 0 MON，平台账户 0 MON；主办方和平台仍需领取测试 MON 后才能完成提款验收。
